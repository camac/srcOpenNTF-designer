...
public class WizardSubPageDataSource extends WizardSubPage {
    ...
    public String getVarName() {
        ...
        if (childDataSourceDataNode != null) {
            IMember member = childDataSourceDataNode.getMember(XSPAttributeNames.XSP_ATTR_VAR);
            if (member instanceof IAttribute) {
                try {
                    varName = childDataSourceDataNode.getValue((IAttribute) member);
                }
                catch (NodeException e) {
                }
            }
    ...
    public String getAction() {
        ...
        if (childDataSourceDataNode != null) {
            IMember member = childDataSourceDataNode.getMember(XSPAttributeNames.XSP_ATTR_ACTION);
            if (member instanceof IAttribute) {
                try {
                    action = childDataSourceDataNode.getValue((IAttribute) member);
                }
                catch (NodeException e) {
                }
            }
    ...
    public String getDocumentId() {
        ...
        if (childDataSourceDataNode != null) {
            IMember member = childDataSourceDataNode.getMember(XSPAttributeNames.XSP_ATTR_DOCUMENT_ID);
            if (member instanceof IAttribute) {
                try {
                    docId = childDataSourceDataNode.getValue((IAttribute) member);
                }
                catch (NodeException e) {
                }
            }
    ...
    public String getComputeWithForm() {
        String computeWithForm = "";
        
        if (childDataSourceDataNode != null) {
            IMember member = childDataSourceDataNode.getMember(XSPAttributeNames.XSP_ATTR_COMPUTE_WITH_FORM);
            if (member instanceof IAttribute) {
                try {
                    computeWithForm = childDataSourceDataNode.getValue((IAttribute) member);
                }
                catch (NodeException e) {
                }
            }
    ...
    public String [][] getViewColumns() {
        ...
        try {
            String[][] columns = getVisibleViewColumns(serverName, dbName, viewName, false);
    ...
    //
    // Helper function to get Visible View Columns
    //
    public static String[][] getVisibleViewColumns(final String server,
            final String database, final String viewName, final boolean sortableOnly) throws DominoImportException {
                        ...
                        while (it.hasNext()) {
                            ...
                            if (viewName.equals(name)) {
                                ...
                                for (int col = 1; col <= columnSize; col++) {
                                    ...
                                    if (shouldViewColBeAdded && !viewCol.isHidden()) {
