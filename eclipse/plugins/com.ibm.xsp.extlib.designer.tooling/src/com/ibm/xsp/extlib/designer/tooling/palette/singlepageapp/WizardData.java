...
public class WizardData {
    ...
    private Element createSubPageElement(Document ownerDocument, DesignerProject designerPrj, WizardDataSub pageData) {
        ...
        // Preload
        if(((WizardSubPageMain)pageData.wizardPage[0]).getSubPagePreLoad()) { 
            FormModelUtil.setAttribute(newPage, IExtLibAttrNames.EXT_LIB_ATTR_PRELOAD, "true"); // $NON-NLS-1$
        }

        // Reset content
        if(((WizardSubPageMain)pageData.wizardPage[0]).getSubPageResetContent()) {
